...
import {
    Check, Sparkles, Zap, ShieldCheck, Crown,
    RefreshCcw, Lock, CreditCard, HelpCircle,
    Rocket, Wand2, ZapIcon, Flame, Star
} from "lucide-react";

// --- 权益配置 (基于 $0.04 成本，保证利润) ---
const plans = [
    {
        name: "Initiate",
    ...
    },
    {
        ...
        btnClass: "bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]",
    },
    {
        name: "Zenith",
...
const faqs = [
    {
        ...
        answer: "Yes. We offer a 7-day 'Quality Guarantee'. If you've used less than 20 credits and are not satisfied with the art style, we'll provide a full refund."
    },
    {
    ...
    },
    {
        ...
        answer: "Elite and Zenith plans include full commercial rights. The Initiate plan is for personal use only."
    }
];
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="text-center max-w-4xl mx-auto mb-24">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-zinc-400 text-[10px] font-black uppercase tracking-[0.3em] mb-8">
                            <Flame className="w-3 h-3 text-orange-500" /> Powered by H100 Clusters
                        </div>
                        <h2 className="text-5xl md:text-8xl font-black italic tracking-tighter mb-8 leading-[0.9]">
                            UNLEASH YOUR <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-500">ZENITH VISION</span>
                        </h2>
                        <p className="text-zinc-500 text-lg md:text-xl font-medium max-w-2xl mx-auto">
                            Transform imagination into pixels with professional-grade AI models.
                            Select a plan to elevate your creative mastery.
                        </p>
                    </motion.div>

                    {/* Toggle Switch */}
                    <div className="mt-12 flex items-center justify-center gap-6">
                        <span className={`text-xs font-black tracking-widest ${!isYearly ? "text-white" : "text-zinc-600"}`}>MONTHLY</span>
                        <button
                        ...
                        <span className={`text-xs font-black tracking-widest ${isYearly ? "text-white" : "text-zinc-600"}`}>
                            YEARLY <span className="text-indigo-400 ml-1 text-[10px] bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-500/20">SAVE 25%</span>
                        </span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto mb-32 items-end">
                    {plans.map((plan, i) => (
                        ...
                        >
                            {/* 流光边框容器 */}
                            <div className={`relative flex flex-col h-full p-10 rounded-[39px] bg-[#0A0A0C]/90 backdrop-blur-3xl border border-white/5 transition-all duration-500 ${plan.border}`}>
                                ...
                                <div className="mb-10">
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{plan.highlight}</span>
                                    </div>
                                    ...
                                    <div className="flex items-baseline gap-2 mb-4">
                                        ...
                                        <span className="text-zinc-600 font-bold text-sm uppercase">/ mo</span>
                                    </div>
                                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 rounded-lg border border-white/10 text-[11px] font-bold text-indigo-400">
                                        <ZapIcon className="w-3 h-3 fill-current" /> {plan.credits} HIGH-SPEED CREDITS
                                    </div>
                                ...
                                <div className="space-y-4 mb-12 flex-grow">
                                    {plan.features.map((feature) => (
                                        <div key={feature} className="flex items-start gap-4">
                                            ...
                                            <span className="text-xs font-bold text-zinc-400 group-hover:text-zinc-200 transition-colors uppercase tracking-tight">{feature}</span>
                                        </div>
                                    ))}
                                </div>

                                {/* 动作按钮 */}
                                <button className={`w-full py-5 rounded-[22px] font-black text-xs uppercase tracking-[0.2em] transition-all active:scale-95 ${plan.btnClass}`}>
                                    Upgrade to {plan.name}
                                </button>
                            </div>

                            {/* 背景发光效果 */}
                            <div className={`absolute inset-0 bg-gradient-to-br ${plan.theme} opacity-0 group-hover:opacity-100 rounded-[40px] blur-3xl transition-opacity duration-700 -z-10`} />
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="max-w-6xl mx-auto mb-40 px-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="flex gap-6 p-10 bg-zinc-900/30 border border-white/5 rounded-[40px] group hover:bg-zinc-900/50 transition-all">
                            ...
                            <div>
                                <h4 className="text-xl font-bold mb-2 tracking-tight italic">IRONCLAD GUARANTEE</h4>
                                <p className="text-sm text-zinc-500 leading-relaxed font-medium">
                                    Not satisfied? We offer a 7-day refund policy for all plans if you've used less than 20 credits. Your satisfaction is our priority.
                                </p>
                            </div>
                        </div>
                        <div className="flex gap-6 p-10 bg-zinc-900/30 border border-white/5 rounded-[40px] group hover:bg-zinc-900/50 transition-all">
                            ...
                            <div>
                                <h4 className="text-xl font-bold mb-2 tracking-tight italic">SECURE INFRASTRUCTURE</h4>
                                <p className="text-sm text-zinc-500 leading-relaxed font-medium">
                                    Payments are processed via Stripe with 256-bit SSL encryption. We never store your sensitive card information.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="max-w-3xl mx-auto">
                    <div className="text-center mb-16">
                        <h3 className="text-4xl font-black italic tracking-tighter uppercase mb-4">Questions?</h3>
                        <div className="h-1 w-12 bg-indigo-500 mx-auto rounded-full" />
                    </div>

                    <div className="space-y-4">
                        {faqs.map((faq, i) => (
                            <div key={i} className="border border-white/5 rounded-[28px] bg-white/[0.02] overflow-hidden transition-all hover:border-white/10">
                                <button
                                    onClick={() => setActiveAccordion(activeAccordion === i ? null : i)}
                                    className="w-full flex items-center justify-between p-7 text-left"
                                >
                                    <span className="font-bold text-sm md:text-base text-zinc-300 uppercase tracking-tight">{faq.question}</span>
                                    <div className={`p-1.5 rounded-full border transition-all duration-300 ${activeAccordion === i ? "bg-indigo-500 border-indigo-500 rotate-180" : "border-white/10 bg-white/5"}`}>
                                        <Plus className={`w-4 h-4 transition-transform ${activeAccordion === i ? "rotate-45" : ""}`} />
                                    </div>
                                </button>
                                <AnimatePresence>
                                    {activeAccordion === i && (
