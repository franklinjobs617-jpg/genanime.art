    ...
    <div className="break-inside-avoid mb-4 group relative bg-[#0f0f0f] border border-white/5 rounded-2xl overflow-hidden hover:border-purple-500/40 transition-all duration-300 flex flex-col shadow-lg shadow-black/40 hover:shadow-purple-500/10">
      ...
      >
        ...
        <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:block">
          <button onClick={onTryStyle} className="bg-purple-600 text-white p-2 rounded-full shadow-lg hover:bg-purple-500 transition active:scale-95">
            <Sparkles size={14} />
          </button>
        </div>
      ...
      {/* 信息区域 */}
      <div className="p-3 md:p-4 flex flex-col bg-gradient-to-b from-[#111] to-[#0a0a0a] cursor-pointer" onClick={onClick}>
          ...
          >
            ...
            <div className="relative w-full lg:flex-1 h-[40vh] lg:h-full bg-[#020202] flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute inset-0 opacity-20 blur-3xl scale-110 pointer-events-none">
                <img src={`/images/prompts/${selectedItem.imageName}.webp`} alt="" className="w-full h-full object-cover" />
              </div>
            ...
            <div className="w-full lg:w-[400px] lg:min-w-[400px] bg-[#0f0f0f] border-l border-white/5 flex flex-col h-[50vh] lg:h-full">
              <div className="p-6 border-b border-white/5">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedItem.tags.map((tag: string) => (
                    <span key={tag} className="text-[10px] font-bold uppercase bg-purple-500/10 text-purple-300 border border-purple-500/20 px-2.5 py-1 rounded-md">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-6 bg-[#0a0a0a] border-t border-white/10 z-20 pb-safe">
                <div className="flex flex-col gap-3">
                  <button onClick={() => handleTryStyle(selectedItem.prompt)} className="w-full py-3.5 rounded-xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 transition shadow-lg shadow-purple-500/20 flex items-center justify-center gap-2 text-white active:scale-95">
                    <Sparkles size={18} fill="currentColor" /> {t('generateButton')}
                  </button>
                  <button onClick={() => handleCopy(selectedItem.prompt, selectedItem.id)} className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold border transition active:scale-95 ${copiedId === selectedItem.id ? "bg-green-500/10 border-green-500/50 text-green-400" : "bg-white/5 border-white/10 hover:bg-white/10 text-gray-300"}`}>
      ...
      <div className="pt-24 pb-20 px-4 md:px-8 max-w-[1600px] mx-auto">
        ...
        <header className="max-w-4xl mx-auto text-center mb-16">
          ...
          <p className="text-gray-400 text-base md:text-lg mb-8 max-w-2xl mx-auto leading-relaxed">
            {t('seoDescription') || "Browse our curated collection of high-quality anime AI art prompts. Perfect for Stable Diffusion, Midjourney, and Niji Journey."}
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-2 relative z-10">
            {["Anime", "Girl", "Cyberpunk", "Kawaii", "Landscape", "Portrait", "Mecha"].map((tag) => (
        ...
        <main className="mb-24">
          ...
          {filteredPrompts.length === 0 && (
             <div className="text-center py-20">
                ...
                <button onClick={() => setSearchQuery("")} className="mt-4 text-purple-400 hover:text-purple-300 underline">Clear search</button>
             </div>
          )}
        </main>
        ...
        <section className="mb-24 py-12 border-t border-white/5">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400 max-w-2xl mx-auto">{t('featuresDesc') || "Curated for high consistency and beautiful aesthetics."}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/5 hover:border-purple-500/20 transition group">
              ...
              <p className="text-sm text-gray-400 leading-relaxed">From Makoto Shinkai landscapes to 90s retro aesthetics, explore a diverse library.</p>
            </div>
            <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/5 hover:border-purple-500/20 transition group">
              ...
              <p className="text-sm text-gray-400 leading-relaxed">Copy our optimized prompts and simply change the character or setting.</p>
            </div>
            <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/5 hover:border-purple-500/20 transition group">
              ...
              <p className="text-sm text-gray-400 leading-relaxed">Includes "masterpiece", "best quality" tags for crispest details.</p>
            </div>
          </div>
        </section>
            ...
            <div className="bg-[#121214] border border-white/10 rounded-2xl p-6 md:p-8 mb-12 overflow-hidden relative">
               ...
               <div className="flex flex-wrap gap-3 font-mono text-sm">
                 ...
                 <div className="bg-purple-500/10 border border-purple-500/30 text-purple-200 px-4 py-3 rounded-lg flex-1 min-w-[140px]">
                   <span className="block text-[10px] uppercase text-purple-500/70 font-bold mb-1">3. Style</span>
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {[
              { q: "Can I use these prompts for Midjourney?", a: "Yes! Optimized for Midjourney, Stable Diffusion, and DALL-E." },
              { q: "What is a negative prompt?", a: "It tells the AI what NOT to include, like 'bad hands' or 'low quality'." },
            ].map((faq, i) => (
